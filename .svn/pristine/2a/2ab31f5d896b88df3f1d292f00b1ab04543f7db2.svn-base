...
/**
 * Driver Class Util
 *	这个工具类用来创建类对象，调用工具方法的时候参数可以传：类标识--类名映射中的类标识或者类名，都可以创建出对应的类对象
 * @author WangYuFeng
 */
public class DriverClassUtil{
    ...
    static {
        ...
        InputStream is = null;
        try {
            is = DriverClassUtil.class.getClassLoader().getResourceAsStream("drivers.properties");
            prop.load(is);
        } catch (IOException e) {
            e.printStackTrace();
        } finally {
     ...
     * @throws ClassNotFoundException if can't find the Class
     */
    public static String getDriverName(Class clazz) throws ClassNotFoundException {
        if (clazz == null) {
            return null;
        } else {
