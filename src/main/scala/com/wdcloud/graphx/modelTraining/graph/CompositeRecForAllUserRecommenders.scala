...
class CompositeRecForAllUserRecommenders(userConf: Map[String, String]) extends Recommender(userConf) { 
   ...
   * 	return: 推荐结果  
   */  
  override def predict(sc: SparkContext, graphModel: Graph[Map[Int, Any], Double]): RDD[(VertexId, Map[Long, Array[(Long, Double)]])] = { 
    ...
    val middleResult = recBasedFriends.union(recBasedCircle) 
