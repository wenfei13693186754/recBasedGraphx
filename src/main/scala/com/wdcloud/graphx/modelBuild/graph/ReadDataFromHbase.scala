...
object ReadDataFromHbase extends Serializable {
   ...
   */
  def readGraphData(@transient sc: SparkContext, conf: Configuration): RDD[(Long, Long, Double, Map[Int, Any], Map[Int, Any])] = {
    conf1 = conf
    ...
    //从hbase中读取用户配置信息
    val edgeTable = conf1.get("user.behavior.table")
    ...
    val hBaseRDD = sc.newAPIHadoopRDD(hbaseConf, classOf[TableInputFormat],
      ...
      classOf[org.apache.hadoop.hbase.client.Result]).repartition(6)
    ...
    val edgesData = hBaseRDD.map { //这里是按行键读取的，也就是下边获得的key是一个一个的行健，不是一个行健组成的数组   行健：用户业务id_物品业务id
      case (_, result) =>
        ...
        //读取行为类型
        val relaScore: Double = conf1.get(bhv_type).toDouble
        //计算顶点之间亲密度
  ...
  /**
   * 读取用户配置信息对应的基础表中的算法的类的全路径名等信息
   */
  def readBasedInfo(@transient sc: SparkContext, @transient conf: Configuration): RDD[(String, String, String)] = {
//获取配置信息
    namespace = conf.get("namespace")
    val tableName = "JANUARY:T_USER_CONF"
    //从用户的配置信息中读出使用的数据源类型、算法、结果保存方式的标示信息
    val dst = conf.get("dataSource.type")
    val algo = conf.get("rec.recommender.class")
    val srt = conf.get("rec.saveReslut.type")
    //生成hbaseConf
    val hbaseConf = HbaseUtil.createHbaseConfObject()
    hbaseConf.set(TableInputFormat.INPUT_TABLE, tableName)
    ...
    if (admin.isTableAvailable(tableName)) {
      ...
      val baseInfo = hBaseRDD.map { //这里是按行键读取的，也就是下边获得的key是一个一个的行健，不是一个行健组成的数组   行健：用户业务id_物品业务id
        x =>
          val result = x._2
          val dstClass = Bytes.toString(result.getValue("INFO".getBytes, dst.getBytes)) //读出用户业务id
          val algoClass = Bytes.toString(result.getValue("INFO".getBytes, algo.getBytes)) //读出用户业务id
          val srtClass = Bytes.toString(result.getValue("INFO".getBytes, srt.getBytes)) //读出用户业务id
          (dstClass, algoClass, srtClass)
      }
