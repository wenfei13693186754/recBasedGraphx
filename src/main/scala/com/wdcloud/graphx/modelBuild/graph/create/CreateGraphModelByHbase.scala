...
class CreateGraphModelByHbase {
  ...
  var conf: Configuration = null
	...
	def createGraph(sc: SparkContext, conf: Configuration): Graph[Map[String, Object], Double] = {  
	  ...
	  //创建边RDD
    val graphData = ReadDataFromHbase.readGraphData(sc, conf)
    val edges = graphData.map(x => Edge(x._1, x._2, x._3)).cache()
 
    //创建顶点RDD
    val vertex = ReadDataFromHbase.readVertexDataByUserAndItemPoint(sc, conf)
    //创建中间图，并缓存  
